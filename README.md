...
- 客户端获得推送服务器地址之后，随即向服务器发起连接，可能会出现以下几种情形：
 ...
 - **路由表**：消息分发模块向推送服务器下发消息时，需要确定应该对哪个推送服务器进行调用，因此推送服务器每次建立起与SDK的长连接后，都会在Redis中存储`deviceId`与推送服务器IP地址之间的映射，后文中称为路由表；
 ...
 - 保持长连接：
    ...
    - 如果超过一定时间阈值都没有收到SDK的心跳信息`PING`，则主动断开连接，删除通道关系表和路由表中对应的表项；
